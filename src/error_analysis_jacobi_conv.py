...
def main():
    ...
    for source_file in source_files:
        ...
        filename = f"conv_jacobi_ground_truth_bc_{noise_lvl:.2f}_noise.mat"
        # filename = f"conv_jacobi_zero_dirichlet_bc_{noise_lvl:.2f}_noise.mat"
        # filename = f"conv_jacobi_zero_neumann_bc_{noise_lvl:.2f}_noise.mat"
        save_results(filename, u, residuals, source, noise_lvl)


def iterate(num_iter, arr, source, kernel, bc_fn, progress, tol=1e-3):
    residuals = []
